...
describe('Patch', () => {
  ...
  it('patch simple', () => {
    ...
    const id = new UUID64();
    const thing1 = { id, color: 'blue', size: 34, sale: true, summary: "summary1" };
    const thing2 = { id, color: 'blue', size: 34, sale: true, summary: "summary1" };
    const color = 'red';
    const size = 42;
    const sale = false;
    ...
    p3h2.apply(thing2);
    expect(thing2).toEqual({ id, color, size: 34, sale: true, summary: "summary1" });
    dbg && cc.tag(msg, 'color', p3h2);

    const p3h3 = new Patch({ id, color, size, sale, summary });
    p3h3.apply(thing2);
    expect(thing2).toEqual({ id, color, size, sale, summary });
