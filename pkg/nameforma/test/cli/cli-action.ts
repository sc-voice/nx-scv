...
describe('CLI: action command', () => {
  ...
  beforeEach(async () => {
    ...
    const { testCmd: tc, nfProgram } = await createTestCmd(
      program,
      tempWorld.worldPath,
    );
    testCmd = tc;

    // Register commands with nfProgram
    taskCmd = program.command('task');
    TaskCommand.registerCommand(taskCmd, nfProgram);

    actionCmd = program.command('action');
    ActionCommand.registerCommand(actionCmd, nfProgram);
  });
  ...
  it('action add to focused task', async () => {
    ...
    output = [];
    {
      const w = await FileRepository.worldFromPath(tempWorld.worldPath);
      const t = await w.loadFuzzy(Task, taskId);
  ...
  it('action delete by index', async () => {
    ...
    let task = await world.loadFuzzy(Task, taskId);
    expect(task).toBeTruthy();
  ...
  describe('action get and set (dotref interface)', () => {
    async function setupTaskWithAction() {
      ...
      const w = await FileRepository.worldFromPath(tempWorld.worldPath);
      const task = await w.loadFuzzy(Task, taskId)!;
