...
const URA = (Unicode as any).RIGHT_ARROW;
  ...
  async process() {
    ...
    while (this.running) {
      try {
        ...
        cc.bad1(`${msg} CRASH`, err.message);
        console.log(msg, err);
        crashed = true;
        await this.stop();
        this.onCrash && this.onCrash(err);
      }
    }
...
class GroupOffsets {
  ...
  constructor(cfg: any = {}) {
    ...
    let partitions = topic.partitions.map((p: any, i: any) => {
        ...
        if (offset < messages.length) {
          ...
          for (; offset < messages.length; offset++) {
            ...
            if (message) {
              ...
              cc.bad1(msg, 'empty message?');
            }
          } // for
          partitions[i].offset = offset; // commit
        } else {
          dbg > 1 && cc.ok1(msg, 'no new messages:', `${topicName}.${i}`);
        }
      } // partitions
    }
...
export class Admin extends Role {
  ...
  async fetchOffsets(args: any = {}) {
    ...
    let offsets = topics.reduce((a: any, topicName: any) => {
      let g10s = _groupOffsetsetsMap[topicName];
      g10s && a.push(g10s);
      return a;
    }, []);
...
export class KRaftNode {
  ...
  _groupOfId(groupId: any, opts: any = {}) {
