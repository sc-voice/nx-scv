 ...
 * Wraps Commander.js to enforce global option ordering and encapsulate all commands
 */
export class CLI {
  ...
  /**
   * Create and configure the Command program with all registered commands
   */
  private createProgram(): Command {
    ...
    program
      ...
      .hook('preAction', (thisCommand: any) => {
        ...
        if (thisCommand.optsWithGlobals().agent) {
    ...
    ReferenceCommand.registerCommand(refCmd);
   ...
   * This ensures global options like -v, -w, --debug are recognized regardless of order
   */
  ...
  /**
   * Parse command-line arguments with preprocessing
   * @param argv - Raw command-line arguments (including 'node' and script name)
   */
  ...
  /**
   * Get the underlying Command instance for direct access if needed
   */
  getProgram(): Command {
    return this.program;
  }
...
// Skip CLI parsing during tests to avoid unhandled errors from vitest arguments
