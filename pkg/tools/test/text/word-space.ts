...
const MN8_MOHAN_JSON = JSON.parse(
  fs.readFileSync(path.join(TEST_DATA, 'mn8_legacy-fr-wijayaratna.json'), 'utf-8'),
);
const MN8_MOHAN = MN8_MOHAN_JSON.text;
const WSTEST_CONFIG = JSON.parse(
  fs.readFileSync(path.join(TEST_DATA, 'mohan-noeismet-ws.json'), 'utf-8'),
);
...
describe('text/word-space', () => {
  ...
  it('custom ctor', () => {
    ...
    let ws = new WordSpace({
      minWord,
      corpusBow: new Vector(corpusBow as any),
      corpusSize,
      wordMap,
    });
    expect(ws.minWord).toBe(minWord);
    expect(ws.transformText('a fox')).toBe('x fox');
    expect(ws.corpusBow).toEqual(corpusBow as any);
    expect(ws.corpusSize).toBe(corpusSize);
  });
  it('string2Vector() FOX', () => {
    let ws = new WordSpace({ normalizeVector: null as any });
  ...
  it('similar-mn8:3.4', () => {
    ...
    let scan = Object.keys(MN8_NOE).reduce(
      ...
      { similar: {} as Record<string, number>, match: '' },
    );
  ...
  it('intersect', () => {
    ...
    let ws = new WordSpace({ normalizeVector: null as any, minWord: 1 });
