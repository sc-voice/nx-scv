...
function mn8MohanApiCache(url: string): { ok: boolean; json: () => Promise<any> } {
  ...
  return {
    ...
    json: async () => {
      ...
      let json = JSON.parse(fs.readFileSync(fpath, 'utf-8'));
      return json;
    },
  };
}
...
const TEST_DOC: TestDocType = {
  ...
  text: [
    ...
    'Jeta, près de la ville de Sāvatthi.</p>',
...
describe('text/legacy-doc', () => {
  ...
  it('create()', () => {
    ...
    expect(l1).toMatch(/^<span.*entendu.*séjournait.*Sāvatthi\.$/);
