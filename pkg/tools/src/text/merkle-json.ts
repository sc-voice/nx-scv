...
function md5blk(s: string): number[] {
  ...
  let md5blks: number[] = [],
    i: number; /* Andy King said do it this way. */
  for (i = 0; i < 64; i += 4) {
    md5blks[i >> 2] =
      s.charCodeAt(i) +
      (s.charCodeAt(i + 1) << 8) +
...
export class MerkleJson {
  ...
  constructor(opts: MerkleJsonOpts = {}) {
    this.hashTag = opts.hashTag || 'merkleHash';
  }

  hash(value: unknown, cached: boolean = true): string {
    if (typeof value === 'string') {
      return md5(value);
    } else if (value instanceof Array) {
      let acc = '';
      acc = value.reduce((a: string, v) => a + this.hash(v, cached), acc);
      ...
      if (cached && obj[this.hashTag]) {
        return obj[this.hashTag] as string;
      }
      let keys = Object.keys(obj).sort();
      let acc = keys.reduce((a: string, k) => {
  ...
  stringify(value: unknown): string {
    if (value instanceof Array) {
      let body = value.reduce((a: string, v) => {
      ...
      let body = keys.reduce((a: string, k) => {
        return a
          ? `${a},"${k}":${this.stringify((value as Record<string, unknown>)[k])}`
          : `"${k}":${this.stringify((value as Record<string, unknown>)[k])}`;
      }, '');
      return `{${body}}`;
    }
