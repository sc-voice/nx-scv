...
import { BilaraPath } from './bilara-path.js';

export class SuttaCentralId {
  declare scid: string;

  constructor(scid?: string | null) {
    if (scid == null) {
  ...
  static basename(fp: string): string {
    return fp.split('/').pop() || '';
  }

  static match(scid: string, pat: string): boolean {
  ...
  static rangeHigh(scid: string): string {
  ...
  static rangeLow(scid: string): string {
    let slashParts = scid.split('/');
    let scidMain = slashParts.shift() || '';
    let suffix = slashParts.join('/');
  ...
  static languages(text: string): string[] {
  ...
  static scidRegExp(pat?: string): RegExp {
    if (!pat) {
      return /.*/;
    }
  ...
  static partNumber(part: string, id: string): number[] {
  ...
  static scidNumbersLow(id_or_path: string): number[] {
    let scid = BilaraPath.pathParts(id_or_path).suid;
    let colonParts = scid.replace(/^[-a-z]*/, '').split(':');
    let dotParts = colonParts.reduce((a, c) => a.concat(c.split('.')), [] as string[]);
  ...
  static scidNumbersHigh(id_or_path: string): number[] {
    let scid = BilaraPath.pathParts(id_or_path).suid;
    let colonParts = scid.replace(/^[-a-z]*/, '').split(':');
    let dotParts = colonParts.reduce((a, c) => a.concat(c.split('.')), [] as string[]);
    let nums = dotParts.reduce((a, n) => {
  ...
  static compareHigh(a: string, b: string): number {
    const msg = 'SuttaCentralId.compareHigh()';
    const dbg = (DBG as any).COMPARE;
  ...
  static compareLow(a: string, b: string): number {
  ...
  get parent(): SuttaCentralId | null {
  ...
  add(...args: number[]): SuttaCentralId {
    ...
    if (colonParts.length > 1) {
      ...
      for (let i = 0; i < dotParts.length; i++) {
        let a = Number(args[i]);
        dotParts[i] = String(i < args.length ? Number(dotParts[i]) + a : 0);
      }
      ...
      for (let i = 0; i < n; i++) {
        let a = Number(args[i]);
        dotParts[i] = String(Number(dotParts[i]) + a);
      }
      id2 = `${dotParts.join('.')}`;
    }
