...
class Vector extends Object {
  ...
  intersect(vec2: Vector = new Vector()): Vector {
    let keys = Object.keys(this);
    return keys.reduce((a, k) => {
      let v1 = this[k];
      let v2 = vec2[k] || 0;
...
interface WordMapTransformerOpts {
  lang?: string;
...
export class WordMapTransformer {
  ...
  normalize: (s: string) => string;
  reWordMap?: Array<{ re: RegExp; rep: string }>;

  // DEPRECATED
  constructor(oWordMap: Record<string, string> = {}, opts: WordMapTransformerOpts = {}) {
  ...
  compileWordMap(): Array<{ re: RegExp; rep: string }> {
    let { wordMap } = this;
...
interface WordSpaceOpts {
  ...
  minWord?: number;
  normalize?: (s: string) => string;
  normalizeVector?: ((v: Vector, scale?: number) => Vector) | null;
  transformText?: (text: string) => string;
  transformer?: WordMapTransformer;
  reWordMap?: Array<{ re: RegExp; rep: string }>;
  wordMap?: Record<string, string>;
}
...
export class WordSpace {
  ...
  transformer!: WordMapTransformer;
  reWordMap?: Array<{ re: RegExp; rep: string }>;
  wordMap?: Record<string, string>; // DEPRECATED
  readonly transformText!: (text: string) => string;

  constructor(opts: WordSpaceOpts = {}) {
    let {
  ...
  static createTfIdf(): WordSpace {
    let minWord = 1;
    let normalizeVector = (v: Vector) => v;
    return new WordSpace({ minWord, normalizeVector });
  }

  static get WordMapTransformer(): typeof WordMapTransformer {
    return WordMapTransformer;
  }

  static get Vector(): typeof Vector {
    return Vector;
  }

  // Golden Ratio fudge factor scales a count of 1 to ~0.8
  static normalizeVector(v: Vector, scale: number = GOLDEN_FUDGE): Vector {
    let vNew = new Vector(v);
    Object.entries(v).forEach((e) => {
      let [key, value] = e;
      vNew[key] = 1 - Math.exp(-(value as number) * scale);
    });

    return vNew;
  }

  addDocument(doc: string): WordSpace {
    let { corpusBow } = this;
    this.corpusSize += 1;
  ...
  inverseDocumentFrequency(word: string, idfWeight?: number): number {
  ...
  countWords(str: string, maxCount?: number): { bow: Vector; words: string[] } {
  ...
  string2Vector(str: string, scale: number = 1): Vector {
    const msg = 'w7e.string2Vector:';
    if (str == null) {
      throw new Error(`${msg} str?`);
    }
    let dbg = 0;
    let { normalizeVector, minWord } = this;
    let sNorm = this.transformText(str);
    let words = sNorm.split(' ');
