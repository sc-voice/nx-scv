...
export class WordVector {
  ...
  get length(): number {
    if ((this as any).$length == null) {
      (this as any).$length = Object.keys(this).length;
    }
  ...
  add(vec2: WordVector): WordVector {
    let keys = Object.keys(vec2);
    return keys.reduce((a, k) => {
      let v1 = (a as any)[k] || 0;
      let v2 = (vec2 as any)[k];
      if (v2) {
        (a as any)[k] = v1 + v2;
      }
      return a;
    }, new WordVector(this as any));
  }

  increment(vec2: WordVector): WordVector {
    let keys = Object.keys(vec2);
    return keys.reduce((a, k) => {
      let v1 = (a as any)[k] || 0;
      let v2 = (vec2 as any)[k];
  ...
  dot(vec2: WordVector): number {
    ...
    return keys.reduce((a, k) => {
      let v1 = (this as any)[k];
      let v2 = (vec2 as any)[k] || 0;

      return a + v1 * v2;
    }, 0);
  }

  scale(c: number): WordVector {
  ...
  hadamardL1(vec2: WordVector = new WordVector()): WordVector {
  ...
  similar(vec2: WordVector): number {
  ...
  andOneHot(vec2: WordVector): WordVector {
    return Object.keys(this).reduce((a, k) => {
      if ((this as any)[k] && (vec2 as any)[k]) {
        (a as any)[k] = 1;
      }
      return a;
    }, new WordVector());
  ...
  orOneHot(vec2: WordVector): WordVector {
